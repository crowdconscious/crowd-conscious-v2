-- ============================================================
-- 151: Two-number vote architecture
-- total_votes = registered interactions only (per market_votes row, non-anonymous)
-- engagement_count = all interactions (registered + anonymous)
-- Community probability uses registered votes only (market_outcomes aggregates)
-- ============================================================

ALTER TABLE public.prediction_markets
  ADD COLUMN IF NOT EXISTS engagement_count integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.prediction_markets.total_votes IS 'Registered vote rows only — community probability signal.';
COMMENT ON COLUMN public.prediction_markets.engagement_count IS 'All vote rows (registered + anonymous) — reach / social proof.';

-- 1) Rebuild outcome aggregates from REGISTERED votes only
UPDATE public.market_outcomes mo
SET
  vote_count = COALESCE((
    SELECT COUNT(*)::integer
    FROM public.market_votes mv
    WHERE mv.outcome_id = mo.id AND COALESCE(mv.is_anonymous, false) = false
  ), 0),
  total_confidence = COALESCE((
    SELECT SUM(mv.confidence)::numeric
    FROM public.market_votes mv
    WHERE mv.outcome_id = mo.id AND COALESCE(mv.is_anonymous, false) = false
  ), 0);

-- 2) Recompute outcome probabilities per market (weight = total_confidence among registered)
DO $$
DECLARE
  r RECORD;
  tw numeric;
...
-- 3) Backfill prediction_markets counters
UPDATE public.prediction_markets pm
SET
  total_votes = COALESCE((
    SELECT COUNT(*)::integer
    FROM public.market_votes mv
    WHERE mv.market_id = pm.id AND COALESCE(mv.is_anonymous, false) = false
  ), 0),
  engagement_count = COALESCE((
    SELECT COUNT(*)::integer
    FROM public.market_votes mv
    WHERE mv.market_id = pm.id
  ), 0);

-- 4) Refresh current_probability from YES outcome (registered-only probabilities)
UPDATE public.prediction_markets pm
...
-- Helper RPCs (optional; main logic stays in execute_market_vote)
CREATE OR REPLACE FUNCTION public.increment_market_engagement(market_id_param uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.increment_market_registered_votes(market_id_param uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- ============================================================================
-- Registered vote RPC: new insert bumps total_votes + engagement_count
-- ============================================================================
...
-- ============================================================================
-- Anonymous vote: insert row + engagement only — does NOT move probability
-- ============================================================================
...
BEGIN
  ...
  SELECT total_votes, engagement_count INTO v_tv, v_eng
  ...
  RETURN jsonb_build_object(
    ...
    'new_probability', (SELECT probability FROM public.market_outcomes WHERE id = p_outcome_id),
    'confidence', p_confidence,
    'total_votes', COALESCE(v_tv, 0),
    'engagement_count', COALESCE(v_eng, 0)
  );
END;
$$;

-- ============================================================================
-- Claim guest vote: move into registered pool — updates outcomes + total_votes
-- ============================================================================
...
BEGIN
  ...
  -- Apply guest vote into registered aggregates (was excluded before)
  UPDATE public.market_outcomes
  SET vote_count = vote_count + 1,
      total_confidence = total_confidence + v_vote.confidence
  WHERE id = v_vote.outcome_id;
  ...
  VALUES (
    p_market_id,
    (SELECT probability * 100 FROM public.market_outcomes WHERE id = v_vote.outcome_id),
    0,
    (SELECT total_votes FROM public.prediction_markets WHERE id = p_market_id)
  );
