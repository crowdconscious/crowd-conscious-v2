-- ============================================================================
-- Multi-outcome markets: contrarian XP formula
-- ============================================================================
-- For multi-outcome markets, XP rewards underdog picks (contrarian bonus).
-- Picking a 5% option earns more XP than picking an 89% favorite.
-- Formula: baseXP 10 + round((1 - outcome_probability) * 15)
-- ============================================================================
...
BEGIN
  ...
  -- Count outcomes: binary (2) vs multi (3+)
  SELECT COUNT(*) INTO v_outcome_count FROM public.market_outcomes WHERE market_id = p_market_id;

  -- XP formula: binary = 5 + (confidence-1); multi = 10 + contrarian bonus
  IF v_outcome_count <= 2 THEN
    v_xp := 5 + (p_confidence - 1);
  ELSE
    -- Contrarian bonus: underdogs earn more (probability stored as 0-1)
  ...
  -- Check and unlock achievements (if function exists)
