-- Phase 1 (UX overhaul): allow confidence = 0 for "No lo sé".
-- 0 is stored on the vote row but excluded from confidence averages
-- (lib/pulse-vote-aggregates isValidConfidence requires 1..10) and adds
-- 0 weight to confidence-weighted probability. Do not impute 5.
--
-- Prior CHECK: confidence >= 1 AND confidence <= 10 (migration 126).

ALTER TABLE public.market_votes
  DROP CONSTRAINT IF EXISTS market_votes_confidence_check;

ALTER TABLE public.market_votes
  ADD CONSTRAINT market_votes_confidence_check
  CHECK (confidence >= 0 AND confidence <= 10);

COMMENT ON COLUMN public.market_votes.confidence IS
  '1..10 stated certainty. 0 = No lo sé (option recorded; excluded from confidence average / weight).';
...
BEGIN
  ...
  END IF;
  -- 0 = No lo sé: store option, add 0 weight to confidence-weighted shares.
  v_conf_weight := CASE WHEN p_confidence >= 1 THEN p_confidence ELSE 0 END;
  ...
  IF FOUND THEN
    ...
    IF v_existing.outcome_id = p_outcome_id THEN
      PERFORM public.update_outcome_confidence(
        p_outcome_id,
        CASE WHEN v_existing.confidence >= 1 THEN v_existing.confidence ELSE 0 END,
        v_conf_weight
      );
    ELSE
      UPDATE public.market_outcomes
      SET vote_count = GREATEST(0, vote_count - 1),
          total_confidence = GREATEST(0, total_confidence - CASE WHEN v_existing.confidence >= 1 THEN v_existing.confidence ELSE 0 END)
      WHERE id = v_existing.outcome_id;

      UPDATE public.market_outcomes
      SET vote_count = vote_count + 1,
          total_confidence = total_confidence + v_conf_weight
...
COMMENT ON FUNCTION public.execute_market_vote IS
  'Cast a vote. Ranked mode supported. confidence 0 = No lo sé (no weight). No XP for volume/option/confidence.';
...
COMMENT ON FUNCTION public.execute_anonymous_market_vote IS
  'Guest vote. confidence 0 = No lo sé (no weight). No XP for volume/option/confidence.';
...
COMMENT ON FUNCTION public.execute_alias_anonymous_market_vote IS
  'Alias anonymous vote. confidence 0 = No lo sé (no weight). No XP for volume/option/confidence.';
