...
const INDUSTRY_TAGS = [
  'Manufactura', 'Tecnología', 'Retail', 'Construcción', 'Alimentos',
  'Transporte', 'Energía', 'Salud', 'Educación', 'Finanzas'
]
...
interface UserModuleBuilderClientProps {
  userId: string
  userProfile: any
}

export default function UserModuleBuilderClient({
  userId,
  userProfile
}: UserModuleBuilderClientProps) {
  ...
  const toggleIndustryTag = (tag: string) => {
    if (module.industryTags.includes(tag)) {
      setModule({ ...module, industryTags: module.industryTags.filter(t => t !== tag) })
    } else {
      setModule({ ...module, industryTags: [...module.industryTags, tag] })
    }
  }
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      const response = await fetch('/api/user/modules/create', {
        method: 'POST',
      ...
      setMessage({ type: 'success', text: 'Módulo guardado como borrador exitosamente!' })
      ...
      setMessage({ 
        type: 'error', 
        text: error instanceof Error ? error.message : 'Error al guardar el borrador' 
      })
    } finally {
      setSaving(false)
    }
  ...
  const handleSubmitForReview = async () => {
    ...
    if (!module.title || !module.description) {
      setMessage({ type: 'error', text: 'Por favor completa el título y descripción del módulo' })
      return
    }

    if (module.lessons.length === 0) {
      setMessage({ type: 'error', text: 'Por favor agrega al menos una lección' })
      return
    }

    const incompleteLessons = module.lessons.filter(l => !l.title || !l.description)
    if (incompleteLessons.length > 0) {
      setMessage({ type: 'error', text: 'Por favor completa todas las lecciones' })
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to submit module')
      }

      setMessage({ type: 'success', text: '¡Módulo enviado para revisión exitosamente!' })
      ...
      setMessage({ 
        type: 'error', 
        text: error instanceof Error ? error.message : 'Error al enviar el módulo' 
      })
    } finally {
      setSubmitting(false)
    }
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-600 mt-1">
                  Comparte tu conocimiento y gana ingresos 💰
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              <button
                ...
                className="px-6 py-3 bg-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-300 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={saving || submitting}
                className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-medium hover:scale-105 transition-transform flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setModule({ ...module, estimatedHours: parseInt(e.target.value) || 0 })}
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-purple-600 focus:outline-none"
                  min="1"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setModule({ ...module, xpReward: parseInt(e.target.value) || 0 })}
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-purple-600 focus:outline-none"
                  min="100"
                  step="100"
                />
              ...
              <div>
                ...
                <input
                  type="number"
                  value={module.basePriceMxn}
                  onChange={(e) => setModule({ ...module, basePriceMxn: parseInt(e.target.value) || 0 })}
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-purple-600 focus:outline-none"
                  min="1000"
                  step="500"
                />
                <p className="text-xs text-slate-500 mt-1">Precio para 1-50 empleados</p>
              </div>

              {/* Price Per 50 */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Precio por 50 adicionales (MXN) *
                </label>
                <input
                  type="number"
                  value={module.pricePer50}
                  onChange={(e) => setModule({ ...module, pricePer50: parseInt(e.target.value) || 0 })}
                  className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-purple-600 focus:outline-none"
                  min="500"
                  step="500"
                />
                <p className="text-xs text-slate-500 mt-1">Precio por cada 50 empleados adicionales</p>
              </div>

              {/* Industry Tags */}
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Industrias Relevantes (Opcional)
                </label>
                  ...
                  {INDUSTRY_TAGS.map(tag => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                        module.industryTags.includes(tag)
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) => updateLesson(lesson.id, { estimatedMinutes: parseInt(e.target.value) || 0 })}
                                  className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:border-purple-600 focus:outline-none"
                                  min="5"
                                />
                              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl p-6 mb-6">
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700">${module.basePriceMxn.toLocaleString()} MXN</span>
                </div>
              </div>
              {module.industryTags.length > 0 && (
                <div className="mt-4">
                  <span className="font-bold text-slate-900 text-sm">Industrias: </span>
                  <span className="text-slate-700 text-sm">{module.industryTags.join(', ')}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-between pt-8 border-t border-slate-200 mt-8">
              ...
              <button
                onClick={handleSubmitForReview}
                disabled={saving || submitting}
                className="px-8 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-bold hover:scale-105 transition-transform flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Eye className="w-5 h-5" />
