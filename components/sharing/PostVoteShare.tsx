...
import { trackShare, shareStoryImage } from '@/lib/share-utils'
 ...
 * and a clipboard copy on desktop. Copy follows the first-person voted
 * variants in docs/SHARE-CARDS-STRATEGY-2026-06-09.md §6 — this component
 * only renders after a vote, so the voted variant is always correct.
 */
export function PostVoteShare({
  ...
  const shareText =
    target.type === 'location'
      ? locale === 'es'
        ? `Ya voté: ¿es ${title} un Lugar Consciente? ${proofLine}Vota tú también: ${url}`
        : `I voted: is ${title} a Conscious Location? ${proofLine}Cast yours: ${url}`
      : locale === 'es'
        ? `Yo ya voté en este Pulse. ${proofLine}¿Tú qué opinas? ${url}`
        : `I just voted on this Pulse. ${proofLine}What do you think? ${url}`
  ...
  const handleStory = async () => {
    if (target.type !== 'market' || storyBusy) return
    setStoryBusy(true)
    try {
      const result = await shareStoryImage(target.marketId, { title, locale })
      if (result !== 'cancelled') {
        trackShare(target, 'story_download', surface, 'png')
      }
    } catch {
      /* story share is best-effort; the link buttons remain */
    } finally {
      setStoryBusy(false)
    }
  }
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {target.type === 'market' && (
          <button
            ...
            disabled={storyBusy}
            className="inline-flex min-h-[40px] items-center gap-2 rounded-lg border border-[#2d3748] bg-transparent px-4 py-2 text-sm font-medium text-slate-200 transition-colors hover:border-emerald-500/40 hover:text-emerald-300 disabled:opacity-60"
          >
            <ImageIcon className="h-4 w-4" />
            {locale === 'es' ? 'Compartir historia' : 'Share story'}
          </button>
        )}
        <button
          type="button"
