-- =====================================================
-- PHASE 7: GAMIFICATION FUNCTIONS (SAFE VERSION)
-- Drops existing functions first, then recreates them
-- Use this if you get "cannot change return type" error
-- =====================================================
...
CREATE OR REPLACE FUNCTION public.calculate_tier(p_xp INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION public.xp_for_next_tier(p_tier INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  WITH ranked_users AS (
    SELECT 
      user_id,
      ROW_NUMBER() OVER (ORDER BY total_xp DESC, updated_at ASC) as new_rank
