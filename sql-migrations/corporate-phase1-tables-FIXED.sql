-- ============================================
-- Corporate Dashboard Phase 1: Core Tables (FIXED)
-- ============================================

-- First, verify corporate_accounts exists
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'corporate_accounts') THEN
    RAISE EXCEPTION 'ERROR: corporate_accounts table does not exist. Please create it first using the earlier migrations.';
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Corporate admins can manage invitations" ON employee_invitations;
DROP POLICY IF EXISTS "Anyone can view their own invitation by token" ON employee_invitations;
DROP POLICY IF EXISTS "Employees can view their own enrollments" ON course_enrollments;
DROP POLICY IF EXISTS "Corporate admins can view all company enrollments" ON course_enrollments;
DROP POLICY IF EXISTS "Employees can update their own enrollments" ON course_enrollments;
DROP POLICY IF EXISTS "System can create enrollments" ON course_enrollments;
DROP POLICY IF EXISTS "Employees can view their own certifications" ON certifications;
DROP POLICY IF EXISTS "Corporate admins can view company certifications" ON certifications;
DROP POLICY IF EXISTS "Anyone can verify certifications by code" ON certifications;
DROP POLICY IF EXISTS "Corporate admins can manage impact metrics" ON impact_metrics;
DROP POLICY IF EXISTS "Employees can view company impact" ON impact_metrics;
DROP POLICY IF EXISTS "Employees can manage their own projects" ON project_submissions;
DROP POLICY IF EXISTS "Corporate admins can view and verify projects" ON project_submissions;
DROP POLICY IF EXISTS "Corporate admins can view activity log" ON corporate_activity_log;
...
DROP TRIGGER IF EXISTS update_invitations_updated_at ON employee_invitations;
...
DROP TRIGGER IF EXISTS update_impact_updated_at ON impact_metrics;
CREATE TRIGGER update_impact_updated_at BEFORE UPDATE ON impact_metrics
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_projects_updated_at ON project_submissions;
...
BEGIN
  ...
  RAISE NOTICE 'Functions: log_corporate_activity, auto_enroll_employee';
  RAISE NOTICE 'Triggers: update_updated_at_column (on 4 tables)';
END $$;
