...
- LLM: Anthropic API (`ANTHROPIC_API_KEY` in Vercel env). Model IDs live in ONE place — `lib/agents/config.ts` `MODELS` — never hardcoded elsewhere. Bulk/panel → `MODELS.FAST` (Haiku 4.5, currently `claude-haiku-4-5-20251001`) (+ **Batch API** for anything async: 50% cost discount). Drafting/synthesis → `MODELS.CREATIVE` (Sonnet 4.5, currently `claude-sonnet-4-5-20250929`). (Verified Aug 2026 against the repo; an earlier draft of this doc said `sonnet-4-6` — the repo constant is the source of truth.)
...
```
**API:** `POST /api/senal-express/draft` (`MODELS.CREATIVE` (Sonnet 4.5) → strict JSON `{asunto, cuerpo_parrafos[], peticion, categoria_normalizada}`) · `POST /api/senal-express/confirm` (PDF render + señal creation + link). Rate limit 3/day per identity.
